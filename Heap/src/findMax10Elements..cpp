...
#define N 500 // total no of elements
#define T 10 // top T elements
#define block 100
...
int main() {
	...
	for(i=block , j= N-T; i< T + block && j<N; i++ , j++){
		if(temp[i] != arr[j])
			cout<<temp[i]<<"failed";
	}
...
void sortArr(){
	int size = T + block;
	int start = 0;
	std::copy(arr + start , arr + start + size , temp);
	std::sort(temp , temp + size);
	start = start + size;

	while(start + block <= N ){
		std::copy(arr + start , arr + start + block , temp);
		std::sort(temp , temp + size);
		start = start + block;
	}

	if(start + block > N && start < N ){
		std::copy(arr + start , arr + N , temp);
		std::sort(temp , temp + size);
	}
	//int i = T+10;

	for(int i=block ; i< T + block ; i++){
		cout<<temp[i]<<' ';
	}
	return ;
}
